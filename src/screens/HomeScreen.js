...
// Import api routes
import { getAllCategories, getAllFishes, getAllFishesPromotion } from "../routes/HomeAndCategoriesScreen/HomeAndCategoriesRoutes";
// Testing data
const posterData = [
    {
        id: 1,
...
export default function Home({ navigation }) {
    ...
    useEffect(() => {
        ...
        const fetchFishesPromotion = async() => {
            const allFishesPromotion = await getAllFishesPromotion();
    ...
    const handleGetAllFishByCategory = (category) => {          
        switch(category) {
            case "All":
                ...
                break;
            default: 
                setFishList(fishData.filter(item => item.ma_loai_ca_info.TenLoaiMatHang === category && !item.KhuyenMai));
                break
        };
                            ...
                            <View style={styles.body}>
                                ...
                                <View style={styles.searchSection}>
                                    ...
                                    <View>
                                        ...
                                        <FlatList
                                            horizontal={true}
                                            data={[...fishList.slice(0, 2), {MaMatHang: 'seeAll', ma_loai_ca_info: {MaLoaiMatHang: "seeAll", TenLoaiMatHang: selectedCategory}}]}
                                            renderItem={({ item }) => <FishView navigation={navigation} item={item} />}
                                ...
                                <View style={styles.promotionSection}>
                                    ...
                                    <FlatList
                                        horizontal={true}
                                        data={[...promotionList.slice(0, 2), {MaMatHang: 'seeAll', ma_loai_ca_info: {MaLoaiMatHang: "promotion", TenLoaiMatHang: "promotionList"}, }]}
                                        renderItem={({ item }) => <FishView navigation={navigation} item={item} />}
                                        keyExtractor={item => item.MaMatHang}
                                    ></FlatList>
                                </View>
