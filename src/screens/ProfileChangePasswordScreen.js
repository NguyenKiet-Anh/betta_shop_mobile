...
// Import api routes
import { changePassword, getUserInfo } from "../routes/ProfileRoutes/ProfileRoutes";
// Main function
export default function ChangePassword({ route, navigation }) {
    ...
    useEffect(() => {
        const fetchPassword = async(id) => {
            const password = await getUserInfo(id);
        ...
        if (currentPassword !== passwordData) {
            ...
            if (newPassword.length < 6) {
                ...
                if (confirmPassword !== newPassword) {
                    ...
                    const response = await changePassword(userInfo.ma_nguoi_dung, newPassword);
                    if (response.success) {
        ...
        <>
        {
            isLoading ? (
                ...
                <SafeAreaView style={styles.container}>
                    ...
                    <InputField title={"Current Password"} value={currentPassword} setValue={(e) => setCurrentPassoword(e)}></InputField>
                    {/* For new password */}
                    <InputField title={"New Password"} value={newPassword} setValue={(e) => setNewPassword(e)}></InputField>
                    {/* For new password confirmation */}
                    <InputField title={"Password Confirmation"} value={confirmPassword} setValue={(e) => setConfirmPassword(e)}></InputField>
                    {/* Submit button */}
                    <View style={styles.submitSection}>
                        <SubmitButton title={"Submit Change"} onPress={() => {handleChangePassword();}}></SubmitButton>
                    </View>                    
                </SafeAreaView>
            )
        }
